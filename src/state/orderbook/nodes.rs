...
#[repr(C)]
#[derive(Debug, Clone, Copy, bytemuck::Pod, bytemuck::Zeroable, anchor_lang::AnchorDeserialize, anchor_lang::AnchorSerialize)]
pub struct FreeNode {
...
impl AnyNode {
    pub fn key(&self) -> Option<u128> {
        match self.case()? {
            NodeRef::Inner(inner) => Some(inner.key.into()),
            NodeRef::Leaf(leaf) => Some(leaf.key.into()),
        }
    }
